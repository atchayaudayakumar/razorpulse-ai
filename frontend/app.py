...
# ==========================================================
    ...
    try:
        ...
        if outcome_response.status_code == 200:
            ...
            recoveries = outcome_response.json()

            if recoveries:

                recovery_options = {
                    (
                        f"Recovery #{recovery['recovery_id']} | "
                        f"Invoice {recovery['invoice_id']} | "
                        f"{recovery['strategy']}"
                    ): recovery
                ...
                selected_label = st.selectbox(
                    "Select Recovery Attempt",
                    list(recovery_options.keys()),
                )

                selected_recovery = recovery_options[
                    selected_label
                ]

                current_status = str(
                    selected_recovery.get(
                        "status",
                        "",
                    )
                ).lower()
                ...
                if st.button(
                    ...
                    try:
                        ...
                        response = requests.post(
                            f"{BACKEND_URL}/api/recovery/"
                            f"{selected_recovery['recovery_id']}/outcome",
                            params={
                                "status": outcome_status,
                                "amount_recovered": amount_recovered,
                                "notes": outcome_notes,
                        ...
                        if response.status_code == 200:
                            ...
                            with col3:
                                st.metric(
                                    "Amount Recovered",
                                    f"₹{float(result.get('amount_recovered', 0) or 0):,.2f}",
                                )

                            st.info(
                                result.get(
                                    "notes",
                                    "No outcome notes provided.",
                                )
                            )
            ...
            st.error(
                f"Recovery API returned "
                f"status {outcome_response.status_code}."
            )
